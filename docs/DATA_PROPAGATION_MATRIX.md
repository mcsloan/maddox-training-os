...
| `local-only` | Allow current-device use; show caveat if resume across devices is expected. | Badge only if local record affects date status. | Show local-only status. | Add sync/local-only attention item. | Show local-only KPI entries as pending/limited evidence when cloud sync has failed or has not run. | No plan mutation. | Include local-only caveat or mark `export-not-supported`. | Show not available across devices yet. | Continue current-device workflow. |
...
- Standalone KPI entries can sync through `session_logs` with `source = kpi_page`; local-only caveats still apply when cloud write fails or a browser-only backup has not synced.
