...
- Branch: `main`.
- Current local checkpoint: `ec283ce` (`fix(plan): render sport loads with date semantics`).
- Local `main` is ahead of `origin/main` by 1. The Gantt fix is committed locally but still needs push/deploy/smoke.
...
- `DEF-GANTT-SPORTLOAD-DURATION-001` is fixed locally: Plan/Gantt now renders Sport Loads as date-specific chips/ranges instead of week-duration Sport Load bars.
...
- `DEF-GANTT-SPORTLOAD-DURATION-001` fixed locally: Plan/Gantt Sport Load rows now use v8.4-derived date chips/ranges. Single-day Sport Loads stay date-specific; camp/travel sequences show actual date ranges.
...
1. Push/deploy `ec283ce`, then run read-only `/plan` smoke for `DEF-GANTT-SPORTLOAD-DURATION-001`.
2. If the warning persists, fix `DEF-REACT-DUPLICATE-KEY-EASY-SPIN-001` with stable contextual React keys; do not deduplicate valid repeated instruction text.
...
- Defects added/updated: `ENV-PREVIEW-DB-001` added/completed; `DEF-029` reopened; `DEF-030`, `DEF-031`, `DEF-032` added; `DEF-GANTT-SPORTLOAD-DURATION-001` fixed and committed locally; `DEF-REACT-DUPLICATE-KEY-EASY-SPIN-001` added; `DEF-ENV-PREVIEW-SUPABASE-MAPPING-001`, `DEF-SUPABASE-STAGING-AUTOPAUSE-001`, `DEF-QA-CODEX-RUNNER-001`, and `DEF-QA-USAGE-LEDGER-001` added; `DEF-ENV-PREVIEW-SUPABASE-MAPPING-001` and `DEF-ENV-PREVIEW-STAGING-OVERRIDE-001` are production-runtime-verified and awaiting fresh Preview runtime verification.
