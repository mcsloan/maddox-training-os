...
1. Review `docs/KPI_CLOUD_SYNC_STAGING_TEST_PLAN.md`.
2. Wire Vercel Preview to staging.
3. Keep Vercel Production on production.
4. Confirm local env still targets staging without displaying secrets.
...
Do not apply the KPI cloud-sync stash until `docs/KPI_CLOUD_SYNC_STAGING_TEST_PLAN.md` has been reviewed. Do not add `.wip/` to git. Do not display or commit `.env.local`, `.env.local.production-backup`, or any Supabase keys.
