...
- Branch: `main`.
- Current clean checkpoint: `8c51cc8` (`fix(plan): render daily gantt with sticky labels`), pushed to `main` and served in production.
...
| Not started | DEF-REACT-DUPLICATE-KEY-EASY-SPIN-001, DEF-QA-USAGE-LEDGER-001, SESSION-UX-001, RECOVERY-READINESS-001, EXPORTS-REPORTING-001, HOCKEY-IQ-001, DEF-008, DEF-009, DEF-010, DEF-011, DEF-012 |
...
- Likely root cause: at least one rendered list appears to derive the React `key` from display text alone; repeated valid training instructions therefore collide.
...
- Next action: after Gantt push/smoke, inspect list renderers for display-text-derived keys and apply a narrow key fix.
- Links / evidence: Terminal 1 dev server warning observed by Mike.
