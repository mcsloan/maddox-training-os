...
- `DEF-REACT-DUPLICATE-KEY-EASY-SPIN-001` is fixed locally: KPI instruction lists now use KPI ID plus instruction index rather than visible instruction text alone, preserving both valid `Easy spin 2 minutes.` entries.
...
| 4.9.1 | DEF-REACT-DUPLICATE-KEY-EASY-SPIN-001 | React duplicate key warning for repeated Easy Spin instruction text | P2 | Completed locally | Fast lane | Review the focused contextual-key fix and checks, then commit if accepted; do not push unless Mike asks. |
...
Current sprint: the accepted daily-scale Gantt is live and documented through `3d4c1d2`. The focused `DEF-REACT-DUPLICATE-KEY-EASY-SPIN-001` contextual-key fix is completed locally and awaiting review/commit.

Next task brief:

- Read `AGENTS.md`, `docs/SESSION_HANDOFF.md`, and this file first.
- Review the locally completed `DEF-REACT-DUPLICATE-KEY-EASY-SPIN-001` fix; commit only if accepted and do not push unless Mike asks.
- Next, verify a fresh Preview deployment uses staging before any Preview write testing.
...
| Completed locally | DEF-REACT-DUPLICATE-KEY-EASY-SPIN-001 |
...
- Next action: address `DEF-REACT-DUPLICATE-KEY-EASY-SPIN-001`; separately verify a fresh Preview runtime uses staging before Preview write testing.
...
- Local fix: both KPI instruction renderers now use `${kpi.id}-instruction-${index}`, providing stable parent/section context plus source order without changing visible content.
...
- Next action: review checks and commit if accepted; then verify a fresh Preview runtime uses staging before Preview write testing.
- Links / evidence: Terminal 1 warning observed by Mike; `components/KPIProtocolDetails.tsx`; `components/SessionKPIForm.tsx`; `components/KPIProtocolDetails.test.tsx`.
...
- Priority: P1
- Status: Production runtime verified / Preview runtime pending
- Lane: Docs-only / environment-safety
- Owner: Mike / Codex
