...
export default async function ExternalLoadPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="mx-auto max-w-3xl"><div className="mb-5"><Link className="text-sm font-bold text-blue" href={`/day/${load.date}`}>← Back to day</Link></div><section className="card mb-6 bg-navy text-white"><ExternalLoadChip type={load.type} provider={load.provider} title={load.title} /><p className="label mt-4 text-lime">Log Today</p><h1 className="mt-2 text-3xl font-black">{load.title}</h1><p className="mt-2 text-slate-200">{formatPlanDate(load.date, { weekday: "long", month: "long", day: "numeric", year: "numeric" })} · {load.provider} · Planned intensity {load.plannedIntensity}/5</p><p className="mt-3 text-sm text-slate-300">Saving an update creates a new immutable today-log snapshot and preserves older entries.</p></section><ExternalLoadForm load={load} /></div>;
}
