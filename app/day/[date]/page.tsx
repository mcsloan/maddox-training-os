...
import { formatPlanDate, getDayTags, getExternalLoadsForDate, getPlanDay, getRelatedVideos, getWorkout, getWorkoutBlock, getWorkoutDrills, isUsableExternalUrl, kpis } from "@/lib/trainingData";
  ...
  return (
    <div className="mx-auto max-w-5xl">
      ...
      <section className="card bg-navy text-white">
        <p className="label text-lime">{day ? `Week ${day.weekNumber} · ${day.phase} · ${day.dayRole}` : "Sport load day"}</p>
        ...
        <div className="mt-4 flex flex-wrap gap-2">{day && <PhaseChip phase={day.phase} />}{externalLoads.map((load) => <ExternalLoadChip key={load.id} type={load.type} provider={load.provider} title={load.title} />)}{tags.map((tag) => <PlanTagChip key={tag} tag={tag} />)}{(externalLoads[0]?.doNotDoRule || day?.doNotDo || "").toLowerCase().includes("hard dryland") && <LoadChip kind="no-hard-dryland" />}{(externalLoads[0]?.doNotDoRule || day?.doNotDo || "").toLowerCase().includes("kpi") && <LoadChip kind="no-kpi-testing" />}</div>
      ...
      {externalLoads.length > 0 && <section className="card mt-6"><h2 className="text-2xl font-black">Sport Loads</h2><div className="mt-4 grid gap-4 md:grid-cols-2">{externalLoads.map((load) => <article className="rounded-2xl border border-rink p-4" key={load.id}><ExternalLoadChip type={load.type} provider={load.provider} title={load.title} /><p className="mt-3 font-black">{load.provider}</p><p className="mt-1 text-sm">{load.startTime}{load.endTime ? `-${load.endTime}` : ""} · {load.plannedDurationMinutes ? `${load.plannedDurationMinutes} min` : "Duration to confirm"} · Intensity {load.plannedIntensity}/5</p><p className="mt-3 text-sm">{load.notes}</p><p className="mt-3 text-sm text-green-800"><strong>Recovery:</strong> {load.recoveryRule}</p><p className="mt-2 text-sm text-red-700"><strong>Do not do:</strong> {load.doNotDoRule}</p><p className="label mt-4">Track after</p><ul className="list-inside list-disc text-sm">{load.trackingQuestions.map((question) => <li key={question}>{question}</li>)}</ul></article>)}</div><ExternalLoadActions loads={externalLoads} /></section>}
