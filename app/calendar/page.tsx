              ...
              <div className="grid gap-4 lg:grid-cols-2">
                {dates.map((date) => {
                  ...
                  return <article className="card border-2 border-transparent transition hover:border-blue" key={date}>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2"><PhaseChip phase={day?.phase || week.phase} />{loads.map((load) => <ExternalLoadChip key={load.id} type={load.type} provider={load.provider} title={load.title} />)}{tags.map((tag) => <PlanTagChip key={tag} tag={tag} />)}{(loads[0]?.doNotDoRule || day?.doNotDo || "").toLowerCase().includes("hard dryland") && <LoadChip kind="no-hard-dryland" />}{(loads[0]?.doNotDoRule || day?.doNotDo || "").toLowerCase().includes("kpi") && <LoadChip kind="no-kpi-testing" />}</div>
