...
export function ExternalLoadForm({ load }: { load: PlannedExternalLoad }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="sticky bottom-16 rounded-2xl bg-ice/95 p-3 backdrop-blur sm:bottom-0"><button className="btn-primary min-h-16 w-full text-lg" disabled={saving} onClick={save}>{saving ? "Saving..." : "Save External Load Log"}</button><p className="mt-2 text-center text-sm font-semibold text-slate-500">{status}</p></div>
    </div>
  );
}
